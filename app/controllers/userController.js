...
let signUpFunction = (req, res) => {
    ...
    let validateUserInput = () => {
        return new Promise((resolve, reject) => {
            ...
            console.log(req.body.adminPasscode);
            ...
            if (req.body.email) {
                if (!validateInput.Email(req.body.email)) {
                    ...
                    reject(apiResponse)
                } else if (req.body.isAdmin === 'true' && req.body.adminPasscode !== process.env.ADMINPW) {
                    let apiResponse = response.generate(true, 'Admin passcode is wrong', 400, null)
                    reject(apiResponse)
                } else {
                    resolve(req)
                }
                ...
                .exec((err, retrievedUserDetails) => {
                    if (err) {
                        ...
                        let newUser = new UserModel({
                            ...
                            userName: req.body.isAdmin === 'true' ? `${req.body.firstName}-admin` : req.body.firstName
                        })
...
//     http://${req.headers.host}${baseUrl}/reset/${newUserObj.resetPasswordToken}`
//   }
